...
def load(code: str, name: str, market: str):
    return analyze.run_for(tickers.StockRef(code=code, name=name, market=market))
...
st.title("📈 종목 분석")
st.caption("CANSLIM 7개 조건 판정 + 현재/예상 밸류에이션 · 데이터 출처: 네이버 금융")

query = st.text_input("종목명 또는 6자리 종목코드",
                      placeholder="예) 삼성전자 · 005930 · 에코프로",
                      label_visibility="collapsed")

if not query.strip():
    st.info("위 칸에 종목명을 입력하면 분석이 시작됩니다.")
    st.stop()

with st.spinner("네이버 금융에서 데이터를 받아오는 중…"):
    candidates = find(query.strip())

if not candidates:
    st.error(f"'{query}' 에 해당하는 국내 종목을 찾지 못했습니다. 이름 일부나 6자리 코드를 입력해 보세요.")
    st.stop()
...
try:
    with st.spinner(f"{ref.name} 분석 중…"):
        a = load(ref.code, ref.name, ref.market)
except Exception as exc:  # 비공식 API라 언제든 바뀔 수 있다
    st.error(f"분석에 실패했습니다: {exc}")
    st.caption("네이버가 데이터 형식을 바꿨거나 일시적인 네트워크 오류일 수 있습니다. "
               "잠시 후 다시 시도해 보세요.")
    st.stop()
...
cols = st.columns(5)
cols[0].metric("현재가",
               f"{snap.price:,.0f}원" if snap.price else "—",
               f"{snap.change:+,.0f}원 ({snap.change_pct:+.2f}%)"
               if snap.change is not None and snap.change_pct is not None else None)
cols[1].metric("시가총액", f"{snap.market_cap/1e12:,.1f}조원" if snap.market_cap else "—")
cols[2].metric("목표주가 평균", f"{snap.target_price:,.0f}원" if snap.target_price else "—",
...
body = (
    ...
    + metric_row("PEG", "peg", "")
    + metric_row("ROE", "roe", "%")
    + metric_row("EPS (12개월)", "eps_ttm", "원", 0)
    + metric_row("매출 (12개월)", "revenue_ttm", "조원", 1, 1e12)
...
st.markdown(
    ...
    f'신뢰도가 한 단계 낮습니다. 네이버는 미래 분기 컨센서스를 하나만 제공합니다.'
    f'</div>',
    unsafe_allow_html=True,
)
...
- **`N`(New)은 신고가 하나가 아닙니다.** 오닐의 N은 신제품·신규 서비스·새 경영진·새로운 산업
  ...
  **② 새로운 재료**를 함께 보고, 둘 다 충족해야 합격으로 봅니다.
  재료는 최근 {window}일 **공시·뉴스·증권사 리포트 제목을 키워드로 자동 분류**한 것입니다.
  기계적 분류라 사람의 판단을 대신할 수 없으니, 표의 근거란에 뜬 제목을 직접 읽어 보세요.
  (같은 사건을 여러 매체가 쓴 기사는 하나로 합칩니다.)
- **`S`는 유통주식수(float)를 반영하지 못합니다.** 무료로 안정적으로 구할 수 없어 거래량 급증으로 대체했습니다.
...
""".replace("{window}", str(a.newness.window_days))
    )

st.markdown(
    f'<div class="note">데이터: 네이버 금융 · '
    f'시세 기준일 {snap.price_date or "—"} · 컨센서스 기준일 {snap.consensus_date or "—"}<br>'
